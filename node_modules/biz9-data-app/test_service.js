...
const {Data_Logic,Data_Url,Data_Field,Data_Response_Field}=require("./");
...
/* -- DEFINE --
 * 1. ping
 * 1. post / (database, table, data_obj)
 * 2. get / (database, table, id)
 * 3. copy / (database, table, id)
 * 4. delete / (database, table, id)
 * 5. post_items / (database, table, data_obj)
 * 6. search / (database, table, search)
 * 7. count / (database, table, search_filter)
 * 8. delete_search / (database, search_obj)
 */

// -- GLOBALZ --
let PARENT = Data_Logic.get(Table.BLANK,'0',{data:{title:'Title '+Str.get_id(),age:'Age '+Str.get_id(),age:'Location '+Str.get_id()}});
let PARENTS_COUNT = 3;
let PARENTS = [];
let PARENTS_QUERY = { $or: [] };
let SEARCH = Data_Logic.get_search(Table.PRODUCT,{id:'69fd5f638accbc0e6563dd21'});
    ...
    it("post", function(done){
        async.series([
            async function(call){
                ...
                [biz_response,biz_data] = await Service.post(url,PARENT.table,PARENT,option);
                response = biz_response;
                PARENT = biz_data;
                Log.w('BIZ-RESPONSE-POST',response);
                Log.w('BIZ-DATA-POST',PARENT);
            },
            function(call){
                // -- assert --
                assert(Str.check_is_null(PARENT.id) === false);
                assert(Str.check_is_null(PARENT.title) === false);
