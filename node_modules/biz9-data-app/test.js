...
const {Data_Logic,Data_Value_Type,Data_Table,Data_Field,Data_Response_Field}=require("./");
// -- other --
const async = require('async');

/* - DEFINE -
 * 1. ping /
 * 2. count / (database, table, search_filter)
 * 3. copy / (database, table, id, option)
 * 4. delete / (database, table, id, option)
 * 5. delete_search / (database, search,option)
 * 6. get / (database,table, id, option)
 * 7. post / (database, table, data, option)
 * 8. post_items / (database, table, data_obj,option)
 * 9. search / (database, table, search,option)
 */

// -- GLOBALZ --
let PARENT = Data_Logic.get(Project_Table.PRODUCT,'69f52ed63d56eafdef8dd7f8');

//9_count - 9_test_count
describe('count', function(){ this.timeout(25000);
    it("_count", function(done){
...
//9_copy - 9_test_copy
describe('copy', function(){ this.timeout(25000);
    it("_copy", function(done){
...
describe('delete', function(){ this.timeout(25000);
    it("_delete", function(done){
        ...
        let data = {};
        let option = {};
        let post_data = Data_Logic.get(Project_Table.PRODUCT,'0');
        async.series([
            async function(call){
                const [biz_response,biz_data] = await Database.get(Data_Config);
                database = biz_data;
...
describe('many_delete_search', function(){ this.timeout(25000);
    it("_many_delete_search", function(done){
...
describe('get', function(){ this.timeout(25000);
    it("_get", function(done){
        ...
        async.series([
            ...
            async function(call){
                // -- GET-START --
                parent = Data_Logic.get(Project_Table.PRODUCT,'0');
                // -- foreigns
    ...
    it("_post", function(done){
        ...
        async.series([
            ...
            async function(call){
                ...
                // --- new ---
                post_data = Data_Logic.get_test_product({title:'Product '+Str.get_id()});
...
describe('many_delete_search', function(){ this.timeout(25000);
    it("_many_delete_search", function(done){
        ...
        let data = {};
        let option = {};
        let post_data = Data_Logic.get_test_parents();
        async.series([
            async function(call){
                const [biz_response,biz_data] = await Database.get(Data_Config);
                database = biz_data;
...
describe('search', function(){ this.timeout(25000);
    it("_search", function(done){
        ...
        async.series([
            ...
            async function(call){
                ...
                const [biz_response,biz_data] = await Data.search(database,search,option);
                Log.w('biz_response',biz_response);
                Log.w('biz_data',biz_data);
